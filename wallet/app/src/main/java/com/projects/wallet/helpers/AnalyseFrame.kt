...
import com.projects.wallet.facerecognition.*
...
class AnalyseFrame(private var context: Context, private val activity: Activity, private var boundingBoxOverlay: BoxBounding) : ImageAnalysis.Analyzer {
    ...
    override fun analyze(image: ImageProxy) {
        ...
        val frameBitmap = BitmapHelper.rotate(
            BitmapHelper.convertImageToBitmap(image.image!!),
        ...
        withContext( Dispatchers.Default ) {
            ...
            for (face in faces) {
                try {
                    ...
                    if(avgScores.minOrNull()!!>0.8){
                        Logger.log("Not detected")
                    ...
                    predictions.add(
                        Prediction(
                            face.boundingBox,
                            detected,avgScores.minOrNull()!!
                        )
                    )
                }
                catch ( e : Exception ) {
                    // If any exception occurs with this box and continue with the next boxes.
                    Log.e( "Model" , "Exception in AnalyseFrame : ${e.message}" )
                    continue
                }
            }
